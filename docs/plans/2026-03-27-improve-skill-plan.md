...
**Goal:** Создать скилл `/improve` для интерактивного улучшения содержимого заметок Obsidian

**Architecture:** Скилл реализуется как `SKILL.md` в директории `skills/improve/` плагина obsidian-brain, следуя установленному паттерну (frontmatter → Setup → Process → Important). Скилл читает заметку, анализирует по 6 операциям, показывает меню с релевантностью, выполняет выбранные операции по одной с превью.

**Tech Stack:** Obsidian CLI, Claude Code tools (Read, Edit, Grep, Glob, Bash)

---

### Task 1: Создать SKILL.md

**Files:**
- Create: `skills/improve/SKILL.md`

- [ ] **Step 1: Создать директорию скилла**

```bash
mkdir -p "C:\Users\yasdr\.claude\plugins\marketplaces\user-config\plugins\obsidian-brain\skills\improve"
```

- [ ] **Step 2: Написать SKILL.md**

Создать файл `skills/improve/SKILL.md` со следующим содержимым:
...
- [ ] **Step 3: Проверить, что файл корректно зарегистрирован**

```bash
obsidian plugin:reload id=obsidian-brain 2>/dev/null || echo "Plugin reload not needed for Claude Code skills"
```

Скиллы Claude Code регистрируются автоматически по frontmatter в SKILL.md. Перезагрузка не требуется — достаточно наличия файла.

- [ ] **Step 4: Commit**

```bash
cd "C:\Users\yasdr\.claude\plugins\marketplaces\user-config\plugins\obsidian-brain"
git add skills/improve/SKILL.md
git commit -m "feat: add improve skill for note content enhancement"
```

---

### Task 2: Обновить interaction-patterns.md

**Files:**
- Modify: `references/interaction-patterns.md:73-79` (секция Commit Convention)

- [ ] **Step 1: Добавить commit message для improve**

В секции «Commit messages by skill» добавить строку:

```
- improve: `refactor: improve content of [note name]`
```

После существующей строки `- rename-tags: \`refactor: rename tags\``
...
git commit -m "docs: add improve skill commit convention"
```
...
**Files:**
- Modify: `README.md:98-99` (перед секцией «Логирование в Daily Note»)

- [ ] **Step 1: Добавить описание /improve в README**

Вставить после секции `/rename-tags` (строка 99) и перед «## Логирование в Daily Note» (строка 101):

```markdown
### /improve — Улучшение содержимого заметки

Интерактивное улучшение текста: структура, форматирование, таблицы, рерайт, конспект, wikilinks.

**Типичный сценарий:**

1. Запускаете `/improve` и указываете заметку
2. Видите меню операций с оценкой релевантности (высокая/средняя/низкая)
3. Выбираете нужные операции
4. Для каждой операции — превью «было → стало» → подтверждение или отмена
...
| # | Операция | Что делает |
|---|----------|-----------|
| 1 | Structure | Заголовки, реорганизация секций, пустые секции |
| 2 | Format | Bold-термины, списки, callouts |
| 3 | Tabulate | Перечисления → markdown-таблицы |
| 4 | Rewrite | Улучшение формулировок, удаление воды |
| 5 | Summarize | TL;DR с ключевыми тезисами |
| 6 | Wikify | Добавление [[wikilinks]] к существующим заметкам |
```
...
### Task 4: Тестирование на заметке «Стрим ТК ПРМ»

- [ ] **Step 1: Запустить /improve на тестовой заметке**

Вызвать `/improve` и указать заметку «Стрим ТК ПРМ». Проверить:

- [ ] Скилл читает 4 reference-файла
- [ ] Скилл читает заметку
- [ ] Анализ выдаёт адекватные оценки релевантности (Structure — высокая из-за пустых секций и строк 122-123; Wikify — средняя из-за PD-модель, Скоринговые модели)
- [ ] Меню отображается корректно
- [ ] При выборе операций — предупреждение о порядке работает
- [ ] Превью «было → стало» показывается для каждого изменения
- [ ] Подтверждение/отмена работает
- [ ] Откат восстанавливает исходный текст
- [ ] Отчёт и Daily Log отображаются

- [ ] **Step 2: Зафиксировать найденные проблемы**

Если тест выявил проблемы в SKILL.md — исправить и закоммитить:

```bash
cd "C:\Users\yasdr\.claude\plugins\marketplaces\user-config\plugins\obsidian-brain"
git add skills/improve/SKILL.md
git commit -m "fix: adjust improve skill after testing"
```
